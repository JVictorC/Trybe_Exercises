...
CREATE TRIGGER addNewMovieInLog
...
CREATE TRIGGER addAnoAtualEmUpdate
	BEFORE INSERT ON movies
    FOR EACH ROW
BEGIN
	SET
		NEW.release_year = YEAR(CURRENT_DATE());
END$$
...
CREATE TRIGGER mudançaDeValor
	BEFORE UPDATE ON movies
    FOR EACH ROW
BEGIN
	SET NEW.ticket_price_estimation = IF(NEW.ticket_price > OLD.ticket_price, 'Incresing', 'Decreasing');
    INSERT INTO BeeMovies.movies_logs (movie_id, executed_action, log_date)
    VALUES(NEW.movie_id, 'UPTADE', NOW());
END $$
...
CREATE TRIGGER adicionarDeleteInLog
